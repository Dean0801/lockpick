...
/// Truncates with hash if the filename would exceed filesystem limits.
fn cache_filename(name: &str, version: &str) -> String {
    ...
    let filename = format!("{safe_name}@{safe_version}.json");
    if filename.len() <= 200 {
        filename
    } else {
        // Use a simple hash to avoid collisions
        let hash = simple_hash(&format!("{name}@{version}"));
        let truncated = &safe_name[..safe_name.len().min(100)];
        format!("{truncated}_{hash:016x}.json")
    }
}
